"use client";

import Link from "next/link";
import { useSidebar } from "./SidebarContext";

// Exact Sidebar Toggle Icon from openai-clone (icon-1.svg)
export function SidebarToggleIcon({
  className,
  isOpen,
}: {
  className?: string;
  isOpen?: boolean;
}) {
  const closedPath =
    "M9.35719 3H14.6428C15.7266 2.99999 16.6007 2.99998 17.3086 3.05782C18.0375 3.11737 18.6777 3.24318 19.27 3.54497C20.2108 4.02433 20.9757 4.78924 21.455 5.73005C21.7568 6.32234 21.8826 6.96253 21.9422 7.69138C22 8.39925 22 9.27339 22 10.3572V13.6428C22 14.7266 22 15.6008 21.9422 16.3086C21.8826 17.0375 21.7568 17.6777 21.455 18.27C20.9757 19.2108 20.2108 19.9757 19.27 20.455C18.6777 20.7568 18.0375 20.8826 17.3086 20.9422C16.6008 21 15.7266 21 14.6428 21H9.35717C8.27339 21 7.39925 21 6.69138 20.9422C5.96253 20.8826 5.32234 20.7568 4.73005 20.455C3.78924 19.9757 3.02433 19.2108 2.54497 18.27C2.24318 17.6777 2.11737 17.0375 2.05782 16.3086C1.99998 15.6007 1.99999 14.7266 2 13.6428V10.3572C1.99999 9.27341 1.99998 8.39926 2.05782 7.69138C2.11737 6.96253 2.24318 6.32234 2.54497 5.73005C3.02433 4.78924 3.78924 4.02433 4.73005 3.54497C5.32234 3.24318 5.96253 3.11737 6.69138 3.05782C7.39926 2.99998 8.27341 2.99999 9.35719 3ZM6.85424 5.05118C6.24907 5.10062 5.90138 5.19279 5.63803 5.32698C5.07354 5.6146 4.6146 6.07354 4.32698 6.63803C4.19279 6.90138 4.10062 7.24907 4.05118 7.85424C4.00078 8.47108 4 9.26339 4 10.4V13.6C4 14.7366 4.00078 15.5289 4.05118 16.1458C4.10062 16.7509 4.19279 17.0986 4.32698 17.362C4.6146 17.9265 5.07354 18.3854 5.63803 18.673C5.90138 18.8072 6.24907 18.8994 6.85424 18.9488C7.17922 18.9754 7.55292 18.9882 8 18.9943V5.0057C7.55292 5.01184 7.17922 5.02462 6.85424 5.05118ZM10 5V19H14.6C15.7366 19 16.5289 18.9992 17.1458 18.9488C17.7509 18.8994 18.0986 18.8072 18.362 18.673C18.9265 18.3854 19.3854 17.9265 19.673 17.362C19.8072 17.0986 19.8994 16.7509 19.9488 16.1458C19.9992 15.5289 20 14.7366 20 13.6V10.4C20 9.26339 19.9992 8.47108 19.9488 7.85424C19.8994 7.24907 19.8072 6.90138 19.673 6.63803C19.3854 6.07354 18.9265 5.6146 18.362 5.32698C18.0986 5.19279 17.7509 5.10062 17.1458 5.05118C16.5289 5.00078 15.7366 5 14.6 5H10Z";

  const openedPath =
    "M9.35719 3H14.6428C15.7266 2.99999 16.6007 2.99998 17.3086 3.05782C18.0375 3.11737 18.6777 3.24318 19.27 3.54497C20.2108 4.02433 20.9757 4.78924 21.455 5.73005C21.7568 6.32234 21.8826 6.96253 21.9422 7.69138C22 8.39925 22 9.27339 22 10.3572V13.6428C22 14.7266 22 15.6008 21.9422 16.3086C21.8826 17.0375 21.7568 17.6777 21.455 18.27C20.9757 19.2108 20.2108 19.9757 19.27 20.455C18.6777 20.7568 18.0375 20.8826 17.3086 20.9422C16.6008 21 15.7266 21 14.6428 21H9.35717C8.27339 21 7.39925 21 6.69138 20.9422C5.96253 20.8826 5.32234 20.7568 4.73005 20.455C3.78924 19.9757 3.02433 19.2108 2.54497 18.27C2.24318 17.6777 2.11737 17.0375 2.05782 16.3086C1.99998 15.6007 1.99999 14.7266 2 13.6428V10.3572C1.99999 9.27341 1.99998 8.39926 2.05782 7.69138C2.11737 6.96253 2.24318 6.32234 2.54497 5.73005C3.02433 4.78924 3.78924 4.02433 4.73005 3.54497C5.32234 3.24318 5.96253 3.11737 6.69138 3.05782C7.39926 2.99998 8.27341 2.99999 9.35719 3ZM6.85424 5.05118C6.24907 5.10062 5.90138 5.19279 5.63803 5.32698C5.07354 5.6146 4.6146 6.07354 4.32698 6.63803C4.19279 6.90138 4.10062 7.24907 4.05118 7.85424C4.00078 8.47108 4 9.26339 4 10.4V13.6C4 14.7366 4.00078 15.5289 4.05118 16.1458C4.10062 16.7509 4.19279 17.0986 4.32698 17.362C4.6146 17.9265 5.07354 18.3854 5.63803 18.673C5.90138 18.8072 6.24907 18.8994 6.85424 18.9488C7.47108 18.9992 8.26339 19 9.4 19H14.6C15.7366 19 16.5289 18.9992 17.1458 18.9488C17.7509 18.8994 18.0986 18.8072 18.362 18.673C18.9265 18.3854 19.3854 17.9265 19.673 17.362C19.8072 17.0986 19.8994 16.7509 19.9488 16.1458C19.9992 15.5289 20 14.7366 20 13.6V10.4C20 9.26339 19.9992 8.47108 19.9488 7.85424C19.8994 7.24907 19.8072 6.90138 19.673 6.63803C19.3854 6.07354 18.9265 5.6146 18.362 5.32698C18.0986 5.19279 17.7509 5.10062 17.1458 5.05118C16.5289 5.00078 15.7366 5 14.6 5H9.4C8.26339 5 7.47108 5.00078 6.85424 5.05118ZM7 7C7.55229 7 8 7.44772 8 8V16C8 16.5523 7.55229 17 7 17C6.44772 17 6 16.5523 6 16V8C6 7.44772 6.44772 7 7 7Z";

  return (
    <svg
      width="18"
      height="18"
      viewBox="0 0 24 24"
      fill="currentColor"
      className={className}
    >
      {isOpen ? (
        <path key="open" fillRule="evenodd" clipRule="evenodd" d={openedPath} />
      ) : (
        <path
          key="closed"
          fillRule="evenodd"
          clipRule="evenodd"
          d={closedPath}
        />
      )}
    </svg>
  );
}

export default function Header() {
  const { toggle, toggleMobileMenu, isMobileMenuOpen, isOpen } = useSidebar();

  return (
    <>
      {/* Fixed header bar - solid background like openai-clone TopBar */}
      <div className="fixed box-border flex h-[54px] justify-end z-[51] px-6 top-0 inset-x-0 md:h-16 md:px-8 bg-[#ffffff] dark:bg-[#000000]">
        <div className="w-full flex justify-between items-center">
          {/* Left: Logo Group */}
          <div className="flex items-center gap-x-3 -ml-1">
            <Link
              aria-label="Home"
              href="/"
              className={`relative hidden md:flex items-center h-full ${
                isMobileMenuOpen
                  ? "opacity-0 md:opacity-100 duration-0"
                  : "opacity-100 transition-opacity duration-200"
              }`}
            >
              <span className="text-[20px] font-bold tracking-tight text-black dark:text-white">
                furqanagwan
              </span>
            </Link>

            {/* Sidebar Toggle - Desktop only */}
            <button
              type="button"
              aria-label="Toggle navigation sidebar"
              onClick={toggle}
              className="text-black/40 dark:text-white/40 bg-transparent hidden min-h-0 min-w-0 text-center p-5 md:block hover:text-black dark:hover:text-white transition-colors"
            >
              <SidebarToggleIcon
                className="w-[18px] h-[18px]"
                isOpen={isOpen}
              />
            </button>
          </div>

          {/* Right: Menu Toggle (mobile) / Contact (desktop) */}
          <div className="flex items-center gap-2">
            {/* Mobile Menu Toggle */}
            <button
              type="button"
              aria-label="Toggle mobile menu"
              onClick={toggleMobileMenu}
              className="text-black/40 dark:text-white/40 bg-transparent text-center p-2 hidden hover:text-black dark:hover:text-white transition-colors"
            >
              <SidebarToggleIcon
                className="w-[18px] h-[18px]"
                isOpen={isMobileMenuOpen}
              />
            </button>

            {/* Contact - Desktop only */}
            <Link
              href="/contact"
              className="hidden md:block text-sm font-medium bg-transparent px-3 py-2 rounded-md hover:bg-black/5 dark:hover:bg-white/5 transition-colors text-black dark:text-white"
            >
              Contact
            </Link>
          </div>
        </div>
      </div>
    </>
  );
}
