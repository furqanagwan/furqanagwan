...
export default function MobileMenu() {
  ...
  return (
    <>
      {/* Mobile Menu Overlay - Theme aware */}
      <div
        className={`
      ...
      >
        ...
        <div className="flex items-center justify-between h-[54px] px-6">
          ...
          <button
            ...
            onClick={closeMobileMenu}
            className="text-black/40 dark:text-white/40 bg-transparent p-2 hover:text-foreground transition-colors"
          >
        ...
        <nav className="px-6 pt-8">
          <ul className="list-none pl-0">
            {navSections.map((section) => {
              ...
              return (
                <li key={section.label} className="mb-2">
                  {hasSubitems ? (
                    <>
                      <button
                        type="button"
                        ...
                        className={`
                          flex items-center justify-between w-full text-base font-bold leading-[22.96px] py-2 bg-transparent border-none cursor-pointer transition-colors
                      ...
                      >
                        <ul className="list-none pl-4 pt-1">
                    ...
                    <Link
                      ...
                      onClick={closeMobileMenu}
                      className={`
                        block text-base font-bold leading-[22.96px] py-2 transition-colors
                        ${sectionActive ? "text-foreground" : "text-muted-foreground hover:text-foreground"}
                      `}
                    >
                      {section.label}
        ...
        <div className="absolute bottom-6 left-6 right-6 flex items-center justify-end">
          <Link
            href="/contact"
            onClick={closeMobileMenu}
            className="text-sm font-medium bg-foreground text-background px-5 py-2.5 rounded-full hover:opacity-80 transition-opacity"
          >
            Contact
          </Link>
        </div>
